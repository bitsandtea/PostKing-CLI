...
exports.brandListCommand = brandListCommand;
exports.displayBrandProfile = displayBrandProfile;
exports.brandInfoCommand = brandInfoCommand;
exports.brandThemesCommand = brandThemesCommand;
exports.brandThemesEditCommand = brandThemesEditCommand;
exports.brandThemesDeleteCommand = brandThemesDeleteCommand;
exports.brandThemesGenerateCommand = brandThemesGenerateCommand;
exports.brandSetCommand = brandSetCommand;
exports.brandOnboardCommand = brandOnboardCommand;
exports.brandCreateCommand = brandCreateCommand;
const client_1 = require("../client");
const config_1 = require("../config");
async function brandListCommand() {
    ...
    try {
        ...
        const activeBrandId = (0, config_1.getBrandId)();
...
function displayBrandProfile(brand) {
    let parsedAudience = {};
    let parsedContext = {};
    ...
    if (parsedAudience.psyche) {
        ...
        if (p.motivations?.length)
            console.log(`DRIVERS:  ${p.motivations.join(", ")}`);
        if (p.frustrations?.length)
            console.log(`FEARS:    ${p.frustrations.join(", ")}`);
    }
    // ─── Audience ──────────────────────────────────────────────────────────────
    console.log("\n[ AUDIENCE DEEP DIVE ]");
    ...
    if (parsedAudience.demographics) {
        ...
        if (d.primaryRoles?.length)
            console.log(`ROLES:    ${d.primaryRoles.join(", ")}`);
        if (d.details) {
            const details = d.details;
            if (details.industry)
                console.log(`INDUSTRY: ${details.industry}`);
            if (details.companySize)
                console.log(`SIZE:     ${details.companySize}`);
    ...
    if (parsedAudience.objections?.length || parsedAudience.trustSources?.length) {
        ...
        if (parsedAudience.objections?.length) {
            ...
            parsedAudience.objections.slice(0, 3).forEach((o) => console.log(`  • ${o}`));
        }
    ...
    if (Object.keys(parsedContext).length) {
        console.log("\n[ PRODUCT CONTEXT ]");
        if (parsedContext.keyFeatures?.length)
            console.log(`FEATURES: ${parsedContext.keyFeatures.join(", ")}`);
        if (parsedContext.technicalAdvantages?.length)
            console.log(`EDGES:    ${parsedContext.technicalAdvantages.join(", ")}`);
        if (parsedContext.marketPosition?.length)
            console.log(`POSITION: ${parsedContext.marketPosition.join(", ")}`);
    }
    // ─── Themes ────────────────────────────────────────────────────────────────
...
async function brandInfoCommand() {
    const client = (0, client_1.createClient)();
...
async function brandThemesCommand() {
    const client = (0, client_1.createClient)();
    const brandId = (0, config_1.getBrandId)();
    if (!brandId) {
...
async function brandThemesEditCommand(themeId, options) {
    const client = (0, client_1.createClient)();
    const brandId = (0, config_1.getBrandId)();
    if (!brandId) {
        console.error("ERROR: No active brand set.");
...
async function brandThemesDeleteCommand(themeId) {
    const client = (0, client_1.createClient)();
    const brandId = (0, config_1.getBrandId)();
    if (!brandId) {
        console.error("ERROR: No active brand set.");
...
async function brandThemesGenerateCommand(options) {
    const client = (0, client_1.createClient)();
    const brandId = (0, config_1.getBrandId)();
    if (!brandId) {
        console.error("ERROR: No active brand set.");
    ...
    try {
        ...
        const initialBrand = (initialRes.data.brands || []).find((b) => b.id === brandId);
        const existingThemeIds = new Set(initialBrand?.themes?.map((t) => t.id) || []);
        // 2. Start generation
        ...
        while (!isReady && attempts < maxAttempts) {
            ...
            if (isTargetOpFinished || isCorrectLegacyOp) {
                ...
                const finalBrand = (finalRes.data.brands || []).find((b) => b.id === brandId);
                // Find new themes by comparing against original snapshot
                const newThemes = finalBrand?.themes?.filter((t) => !existingThemeIds.has(t.id)) || [];
...
function brandSetCommand(brandId) {
    (0, config_1.setConfig)({ brandId });
    console.log(`SUCCESS: Active brand set to: ${brandId}`);
    console.log("All subsequent commands will use this brand workspace.");
}
async function brandOnboardCommand(websiteUrl, options) {
    const client = (0, client_1.createClient)();
    ...
    try {
        ...
        while (!isReady && attempts < maxAttempts) {
            ...
            if (ready || status === "completed") {
                ...
                const brands = finalRes.data.brands || [];
                const brand = brands.find(b => b.id === brandId);
                if (brand) {
                    displayBrandProfile(brand);
                }
...
async function brandCreateCommand(name, options) {
    const client = (0, client_1.createClient)();
    console.log(`\n🛠  Creating brand '${name}'...`);
    try {
        ...
        // Auto-set as active brand
        (0, config_1.setConfig)({ brandId: brand.id });
